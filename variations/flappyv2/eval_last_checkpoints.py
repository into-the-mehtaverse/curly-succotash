...
Examples (from repo root):
  uv run python -m variations.flappyv2.eval_last_checkpoints
  uv run python -m variations.flappyv2.eval_last_checkpoints --run-id 177087020156 --last 5 --episodes 50
  uv run python -m variations.flappyv2.eval_last_checkpoints --difficulty 0.7
"""
...
def main():
    ...
    parser.add_argument("--run-id", type=str, default=None, help="Experiment run id under variations/flappyv2/experiments/")
    ...
    if run_id is None:
        raise SystemExit("No runs found in variations/flappyv2/experiments/")
    ...
    policy = make_flappyv2_lstm_policy(vecenv.driver_env).to(args.device)
