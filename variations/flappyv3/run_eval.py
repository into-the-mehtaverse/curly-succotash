"""
Eval a Flappy v3 policy. Uses the Flappy v3 C binding at a fixed
difficulty (default 1.0: pure uniform [0.25, 0.75], matching standard eval).

Run from repo root:

  uv run python -m variations.flappyv3.run_eval
  uv run python -m variations.flappyv3.run_eval --model variations/flappyv3/experiments/<run_id>/model_009765.pt
  uv run python -m variations.flappyv3.run_eval --episodes 50 --no-render
  uv run python -m variations.flappyv3.run_eval --difficulty 1.0
...
from variations.flappyv3.train import make_flappyv3_lstm_policy
...
def _init_state(policy, batch, device):
    h = getattr(policy, "hidden_size", 128)
    return {
        "lstm_h": torch.zeros(batch, h, device=device),
        "lstm_c": torch.zeros(batch, h, device=device),
    }
...
def run_episode(vecenv, policy, device, seed=None):
    ...
    state = _init_state(policy, vecenv.num_agents, device)
    steps = 0
    pipes_passed = 0
    with torch.no_grad():
        while True:
...
def main():
    parser = argparse.ArgumentParser(description="Eval Flappy v3 policy")
    parser.add_argument(
        "--model",
        type=str,
        default=None,
        help="Path to checkpoint .pt (default: latest in variations/flappyv3/experiments/)",
    )
    ...
    policy = make_flappyv3_lstm_policy(driver).to(args.device)
    ...
    state = _init_state(policy, vecenv.num_agents, args.device)
    total_pipes = 0
    ep_pipes = 0
    with torch.no_grad():
        while True:
